...
namespace ConnectFourMode{
	...
	ConnectFourPlus::~ConnectFourPlus(){//delete the dynamic array from heap
		for(int i=0;i<getWidth();i++){
			delete [] gameBoard[i];
		}
		delete[] gameBoard;
	}
	int ConnectFourPlus::control(int playerNo,int loop){
		int x,y;
		char player;
		int count = 0;
	...
	int ConnectFourPlus::playUserOne(string& save1,string& load1){
		int choice,i,temp;
		int winner;
		int cont;
		char harf;
	...
	int ConnectFourPlus::playUserTwo(string& save3,string& load3){
	...
	int ConnectFourPlus::playComputer(){
		char ch;
		int cont;
		int row;
		int column;
	...
	void ConnectFourPlus::SaveGame(string& save4){
	   char letter;
	   int k = 0;
	...
	void ConnectFourPlus::LoadGame(string& load4){
	  	string satir;
